...
TARGET_COLORS = {
    'Attrition':'#ff6b6b','PerformanceRating':'#f59e0b',
    'AbsentDays':'#00a8ff','PromotionLikelihood':'#00d4a1',
}
...
def show():
    ...
    if imp_data:
        ...
        fig_heat = px.imshow(
            imp_df.T, text_auto='.3f', aspect='auto',
            color_continuous_scale=[[0,'#0c1220'],[0.5,'#1a3a4a'],[1,'#00d4a1']],
            labels=dict(x='Feature', y='Target', color='Importance')
        )
        fig_heat.update_layout(**PT, height=280,
    ...
    with sel_col1:
        target_sel = st.selectbox("Prediction target", list(TARGETS.keys()))
    with sel_col2:
        model_sel  = st.selectbox("Model", list(results[target_sel].keys()))
    ...
    with col1:
        ...
        if hasattr(model, 'feature_importances_'):
            ...
            fig = px.bar(imp.tail(12), orientation='h',
                         color=imp.tail(12).values,
                         color_continuous_scale=[[0,'#1a2540'],[1,color]])
            fig.update_traces(marker_line_width=0)
            fig.update_layout(**PT, height=370, coloraxis_showscale=False)
            st.plotly_chart(fig, use_container_width=True)
        elif hasattr(model, 'coef_'):
            coefs     = model.coef_
            coef_vals = np.abs(coefs).mean(axis=0) if coefs.ndim>1 else np.abs(coefs)
            imp = pd.Series(coef_vals, index=FEATURE_COLS).sort_values(ascending=True)
            fig = px.bar(imp.tail(12), orientation='h',
                         color=imp.tail(12).values,
                         color_continuous_scale=[[0,'#1a2540'],[1,'#a855f7']])
            fig.update_traces(marker_line_width=0)
            fig.update_layout(**PT, height=370, coloraxis_showscale=False)
            st.plotly_chart(fig, use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col2:
        st.markdown(f'<div class="chart-card"><div class="chart-title">Prediction Error Analysis</div>', unsafe_allow_html=True)
        y_te   = mres['y_te'].values
        y_pred = mres['y_pred']
        if task == 'reg':
            residuals = y_te - y_pred
            fig_res   = px.scatter(x=y_pred, y=residuals, opacity=0.55,
                                   color=np.abs(residuals),
                                   color_continuous_scale=[[0,'#10b981'],[0.5,'#f59e0b'],[1,'#ff6b6b']],
                                   labels={'x':'Predicted','y':'Residuals'})
            fig_res.add_hline(y=0, line_dash='dot', line_color='rgba(255,255,255,0.15)')
            fig_res.update_traces(marker=dict(size=4, line=dict(width=0)))
            fig_res.update_layout(**PT, height=320, coloraxis_showscale=False)
            st.plotly_chart(fig_res, use_container_width=True)
            mc1, mc2 = st.columns(2)
            mc1.metric("MAE", f"{np.abs(residuals).mean():.2f} days")
            mc2.metric("R²",  f"{mres['r2']:.3f}")
        else:
            classes   = sorted(np.unique(y_te))
            class_acc = [{'Class':str(c),
                          'Accuracy':(y_pred[y_te==c]==c).mean(),
                          'Count':int((y_te==c).sum())}
                         for c in classes if (y_te==c).sum()>0]
            if class_acc:
                df_ca  = pd.DataFrame(class_acc)
                fig_ca = px.bar(df_ca, x='Class', y='Accuracy',
                                color='Accuracy', text=df_ca['Accuracy'].round(2),
                                color_continuous_scale=[[0,'#1a2540'],[1,color]])
                fig_ca.update_traces(textposition='outside', textfont_size=10,
                                     marker_line_width=0)
                fig_ca.update_layout(**PT, height=300, yaxis_range=[0,1.15],
                                      coloraxis_showscale=False)
                st.plotly_chart(fig_ca, use_container_width=True)
            mc1, mc2 = st.columns(2)
            mc1.metric("Accuracy", f"{mres['accuracy']:.3f}")
            if mres.get('auc'):
                mc2.metric("AUC-ROC", f"{mres['auc']:.3f}")
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    sweep_feat = st.selectbox("Sweep feature", FEATURE_COLS, index=6)

    if sweep_feat in ['Gender','Department','Role','Education']:
        st.markdown('<div class="info-box">Select a numeric feature for the sensitivity sweep.</div>',
                    unsafe_allow_html=True)
    else:
        df_base    = generate_data(100)
        df_enc, _  = encode_df(df_base)
        X_base     = pd.DataFrame(scaler.transform(df_enc[FEATURE_COLS]), columns=FEATURE_COLS)
        ...
        for val in sweep_vals:
            X_orig             = pd.DataFrame(scaler.inverse_transform(X_base), columns=FEATURE_COLS)
            X_orig[sweep_feat] = val
            X_rsc              = pd.DataFrame(scaler.transform(X_orig), columns=FEATURE_COLS)
            for target, task_t in TARGETS.items():
                ...
                if task_t=='clf' and hasattr(m,'predict_proba'):
                    ...
                    sweep_res[target].append(p[:,1].mean() if p.shape[1]==2 else m.predict(X_rsc).mean())
                else:
                    sweep_res[target].append(m.predict(X_rsc).mean())

        fig_sw = go.Figure()
        for target, vals in sweep_res.items():
            c = TARGET_COLORS[target]
            fig_sw.add_trace(go.Scatter(
                x=sweep_vals, y=vals, mode='lines',
                name=TARGET_META[target]['label'],
                line=dict(color=c, width=2.5),
                fill='tozeroy',
                fillcolor=c.replace(')', ',0.04)').replace('rgb','rgba') if 'rgb' in c else f"{c}0a",
            ))
        fig_sw.update_layout(**PT, height=360,
